...
const FlexContent = ({ ifExists, 
  endpoint: { title, img, text, btn, url, heading },
}) => {
  return (
    <>
      <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' : 'flex-row'}`}>
        <div className="max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center">
          <h1 className="text-4-xl sm:text-3xl font-bold text-gradient">{heading}</h1>
          ...
          <a href={url} className=" flex items-center" target={`_blank`} role="button">
            <button type="button" className="button-theme bg-slate-900 shadow-slate-900 text-slate-100 py-1.5 hover:translate-x-1 hover:translate-y-1">
              {btn}
            </button>
          </a>
        </div>
        <div className="flex items-center justify-center max-w-xl relative lg:max-w-none w-full">
          <img src={img} alt={`img/${heading}`} className={`w-auto object-fill transitions-theme ${ifExists ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6  hover:-rotate-12' : 'h-72 lg:h64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12'}`} />
        </div>
      </div>
    </>
  );
